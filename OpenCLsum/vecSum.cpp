...
int main(int argc, char* argv[])
{
	...
	for (int n = 1000; n >= 1; n--)
	{
		...
		if (n < globalSize) {
			length = n;
			loops = static_cast<size_t>(ceil(log(n) / log(localSize)));
		}
		else {
			loops = static_cast<size_t>(ceil(log(globalSize) / log(localSize)));
			length = globalSize;
		}
		// Create a context  
		...
		for (size_t k = 0; k < loops; mode = !mode, k++) {
			...
			err |= clSetKernelArg(kernels[k], 3, sizeof(float), &length);
		...
		//std::cout << "elapsed time for Parallel reduction approach: " << elapsed_seconds.count() << "s\n";
		...
		// print serialize host sum of input vector

		if (abs(sum - h_sum[0]) > 0.001)
		{
			std::cout << "CPU: " << sum << std::endl;
			std::cout << "GPU: " << h_sum[0] << std::endl;
			std::cout << "Test failed! Size of: " << n << std::endl;
			__debugbreak();
		}
		else
		{
			std::cout << "Test passed! Size of: " << n << std::endl;
		}
