...
int main(int argc, char* argv[])
{
	...
	d_sum = clCreateBuffer(context, CL_MEM_READ_WRITE, h_sum.size() * sizeof(float)*ceil(n / localSize), NULL, NULL);

	// Write our data set into the input array in device memory
	err = clEnqueueWriteBuffer(queue, d_a, CL_TRUE, 0, h_a.size() * sizeof(float), h_a.data(), 0, NULL, NULL);
	err = clEnqueueWriteBuffer(queue, d_sum, CL_TRUE, 0, h_sum.size() * sizeof(float)*ceil(n / localSize), h_sum.data(), 0, NULL, NULL);
